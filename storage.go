...
type Storage interface {
	...
	Save(st pb.HardState, ents []pb.Entry) error
	// SaveSnap function saves snapshot to the underlying stable storage.
	SaveSnap(snap pb.Snapshot) error
	...
	Release(snap pb.Snapshot) error
	// Sync WAL
	Sync() error
}
...
func (st *storage) SaveSnap(snap pb.Snapshot) error {
...
func (st *storage) Release(snap pb.Snapshot) error {
...
func ReadWAL(lg *zap.Logger, waldir string, snap pb.Snapshot, unsafeNoFsync bool) (w *log.WAL,
	wmetadata []byte, st pb.HardState, ents []pb.Entry, records []interface{}) {
	var err error
